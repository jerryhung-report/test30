...
<template>
  <div class="max-w-5xl mx-auto py-12 px-6 animate-fadeIn space-y-20 pb-64 sm:pb-48">
    ...
    <div class="fixed bottom-4 sm:bottom-6 left-0 right-0 z-50 px-4">
      <div class="max-w-4xl mx-auto bg-slate-900/95 backdrop-blur-xl text-white py-3 sm:py-6 px-5 sm:px-8 rounded-[2rem] sm:rounded-[3rem] shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-8 border border-white/10">
        <div class="flex items-center gap-4 sm:gap-8 w-full sm:w-auto">
          <div class="relative p-2.5 sm:p-4 bg-[#D21118]/30 rounded-xl sm:rounded-2xl flex items-center justify-center">
            <ShoppingCart :size="20" class="text-[#D21118] sm:w-7 sm:h-7" />
            <span class="absolute -top-1 -right-1 bg-[#D21118] w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[9px] sm:text-[10px] font-black">{{ selected.length }}</span>
          </div>
        ...
        <div class="flex gap-2 sm:gap-4 w-full">
          <button 
            @click="$emit('reset')"
            class="px-4 sm:px-8 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black bg-[#94181C] hover:bg-[#7a1417] text-white text-[11px] sm:text-sm flex-1 flex items-center justify-center gap-2 transition-colors shadow-lg shadow-[#94181C]/20"
          >
            <RotateCcw :size="14" class="sm:w-4 sm:h-4" /> 重新分析
          </button>
          <button 
            @click="openSubscription"
            class="px-6 sm:px-12 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black bg-[#D21118] text-white hover:bg-[#b00e14] text-[11px] sm:text-sm flex-[1.5] flex items-center justify-center gap-2 shadow-lg shadow-[#D21118]/20 transition-all"
          >
            立即申購
          </button>
        </div>
